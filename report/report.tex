...
\begin{algorithm}
	...
	\STATE $\mathbf{y_0} \leftarrow \mathcal{S}(f_1)$ \COMMENT{Select object, store location}
	\STATE $\mathbf{q} \leftarrow \mathcal{M}(f_1, \mathbf{y_0})$ \COMMENT{Create object model}
	\FOR{$i = 1$ to $n$}
...
\caption{Each graph shows the performance of the six different colormodels for
a number of bins on the soccer movie.}
\label{fig:soccer}
\end{figure}
