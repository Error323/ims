...
we annotated by hand is shown in table \ref{tbl:error}. Note that using
normalized $rgb$ results in the highest accuracy.

In figure \ref{fig:soccer} and \ref{fig:cheetah} we compared the performance of
...
cheetah's head respectively. All graphs show a very distinctive pattern on the
color models that lost track. In the socer domain (figure \ref{fig:socer}) this
is the result of occlusion caused by a white player. The white player walks in
front of the orange player (the target) and pushes the model away at which
point it finds a new orange player and starts tracking him. In the nature
domain (figure \ref{fig:cheetah}) this is the moment where the background is
classified as the target object and the tracker starts tracking the background.
