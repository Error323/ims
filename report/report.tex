...
histogram in some color space, often smoothed by a kernel [BRAM: Ik begrijp niet
wat je bedoelt met smoothed]. The tracking
...
iterations. [BRAM: Wat zijn dat? Mean shift operations?] The \emph{Object Model}
is a color histogram defined in some
color-space on which a kernel is applied [BRAM: Hoe wordt die kernel toegepast
op het histogram?]. For our implementation we chose the
Epanechnikov kernel, because of its nice properties. [BRAM: Welke zijn dat?] The
distinction between the \emph{Object Model} and the target candidates is defined
by a metric in color variation. For this metric the Bhattacharyya distance is
chosen. 

\subsection{Epanechnikov Kernel}
The \emph{Object Model} is acquired by the histogram of a square certain [BRAM: certain square?] region
...
Epanechnikov kernel, because its derivative reduces to a constant [BRAM: Als de afgeleide constant is is het een rechte lijn toch?] resulting in
